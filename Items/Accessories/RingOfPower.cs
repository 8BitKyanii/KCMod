...
namespace KCMod.Items.Accessories
{
	...
	public abstract class RingOfPower : ModItem
	{
		...
		public override bool CanEquipAccessory(Player player, int slot) {
			...
			// Therefore we go through each accessory slot ignoring vanity slots using FindDifferentEquippedRingOfPower()
			...
			if (slot < 10) // This allows the accessory to equip in vanity slots with no reservations
			{
				...
				int index = FindDifferentEquippedRingOfPower().index;
				if (index != -1) {
					return slot == index;
				}
			}
		...
		public override void ModifyTooltips(List<TooltipLine> tooltips) {
			...
			Item accessory = FindDifferentEquippedRingOfPower().accessory;
			if (accessory != null) {
		...
		public override bool CanRightClick() {
			...
			// even in vanity slots. For this, FindDifferentEquippedRingOfPower() doesn't find these items
			...
			// Only allow right clicking if there is a different RingOfPower equipped
			if (FindDifferentEquippedRingOfPower().accessory != null) {
				return true;
			}
		...
		public override void RightClick(Player player) {
			...
			var (index, accessory) = FindDifferentEquippedRingOfPower();
			if (accessory != null) {
		...
		protected (int index, Item accessory) FindDifferentEquippedRingOfPower() {
			int maxAccessoryIndex = 5 + Main.LocalPlayer.extraAccessorySlots;
			for (int i = 3; i < 3 + maxAccessoryIndex; i++) {
				...
				// "is RingOfPower" is a way of performing pattern matching
				// Here, inheritance helps us determine if the given item is indeed one of our RingOfPower ones
				if (!otherAccessory.IsAir &&
					!item.IsTheSameAs(otherAccessory) &&
					otherAccessory.modItem is RingOfPower) {
	...
	// Here we add our accessories, note that they inherit from RingOfPower, and not ModItem


	public class YellowRingOfPower : RingOfPower
	{
		public override void SetStaticDefaults() {
			Tooltip.SetDefault("'You feel the power coarsing through your veins...'");
		}
